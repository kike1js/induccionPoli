...
let indiceActual = parseInt(localStorage.getItem('indice_experimental'));
if (isNaN(indiceActual) || indiceActual < 0) {
    indiceActual = 0;
}

let respuestasGuardadas = safeParse('respuestas_experimentales');
let preguntasMarcadas = safeParse('marcadas_experimentales');
let tiempoRestante = parseInt(localStorage.getItem('tiempo_experimental')) || 3600; 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof reactivosExperimentales === 'undefined' || !reactivosExperimentales || reactivosExperimentales.length === 0) {
        console.error("CRÍTICO: No se pudo cargar el arreglo reactivosExperimentales.");
    ...
    if (indiceActual >= reactivosExperimentales.length) {
...
function renderizarPregunta(indice) {
    const reactivo = reactivosExperimentales[indice];
    if (!reactivo) return;

    localStorage.setItem('indice_experimental', indice);

    // Añadimos el ícono visual temático al cargar la materia
    if (DOM.etiquetaMateria) DOM.etiquetaMateria.innerHTML = `<i class="fas fa-microscope mr-1"></i> ${reactivo.materia || "Ciencias Experimentales"}`;
    ...
    if (DOM.contenedorOpciones) {
        ...
        if (reactivo.opciones && Array.isArray(reactivo.opciones)) {
            reactivo.opciones.forEach(opcion => {
                ...
                // Alineación condicional de imágenes
...
function guardarRespuesta(indice, letra) {
    ...
    localStorage.setItem('respuestas_experimentales', JSON.stringify(respuestasGuardadas));
    
    if (preguntasMarcadas[indice]) {
        delete preguntasMarcadas[indice];
        localStorage.setItem('marcadas_experimentales', JSON.stringify(preguntasMarcadas));
    }
...
function actualizarContadorProgreso() {
    ...
    const totales = reactivosExperimentales.length;
...
function generarMapaReactivos() {
    ...
    reactivosExperimentales.forEach((_, i) => {
        const btn = document.createElement('button');
        btn.id = `btn-mapa-${i}`;
        btn.innerText = i + 1;
        btn.onclick = () => {
...
function configurarBotonesNavegacion() {
    ...
    if (DOM.btnSiguiente) {
        DOM.btnSiguiente.onclick = () => {
            if (indiceActual < reactivosExperimentales.length - 1) {
                indiceActual++;
                renderizarPregunta(indiceActual);
            }
        };
    ...
    if (DOM.btnFinalizar) {
        DOM.btnFinalizar.onclick = () => {
            const confirmacion = confirm("¿Estás seguro de que deseas terminar tu examen de Experimentales? Ya no podrás cambiar tus respuestas.");
            if (confirmacion) {
...
function actualizarInterfazNavegacion() {
    ...
    if (DOM.btnSiguiente) {
        DOM.btnSiguiente.disabled = indiceActual === reactivosExperimentales.length - 1;
        DOM.btnSiguiente.style.opacity = indiceActual === reactivosExperimentales.length - 1 ? "0.5" : "1";
    }
}
...
function iniciarReloj() {
    const intervalo = setInterval(() => {
        ...
        localStorage.setItem('tiempo_experimental', tiempoRestante); 
...
async function autoGuardarEnServidor() {
    ...
    const payload = {
        ...
        advertencias: parseInt(localStorage.getItem('advertencias_experimentales')) || 0,
    ...
    try {
        ...
        console.log(`☁️ [Experimentales] Auto-Guardado en la nube (${tiempoRestante}s).`);
    } catch (error) {
...
async function finalizarExamen(motivo = "terminado_por_usuario") {
    ...
    const tiempoSobrante = parseInt(localStorage.getItem('tiempo_experimental')) || 0;
    const advertencias = parseInt(localStorage.getItem('advertencias_experimentales')) || 0;
    const esTrampa = motivo === "trampa";

    const payload = {
        boleta: boletaUsuario,
        area_examen: "ciencias experimentales", 
    ...
    try {
        ...
        // Limpieza profunda local para EXPERIMENTALES
        localStorage.removeItem('respuestas_experimentales');
        localStorage.removeItem('tiempo_experimental');
        localStorage.removeItem('advertencias_experimentales');
        localStorage.removeItem('marcadas_experimentales');
        localStorage.removeItem('indice_experimental');
